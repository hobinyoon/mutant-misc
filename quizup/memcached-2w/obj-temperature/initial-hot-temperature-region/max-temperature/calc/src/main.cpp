...
namespace Parallel {
	...
	vector<double> _initial_max_temperature;
	mutex _lock;
	...
	void MergeTemperatures(const map<long, vector<UtAgeCnts*> >& oid_u_ac) {
		...
		for (auto i: oid_u_ac)
			for (auto j: i.second)
				_initial_max_temperature.push_back(j->initial_peak_max_temp);
	}
	...
	void GenStat() {
		...
		{
			Cons::MT _("Generating stat and creating a CDF file ...");
			ofstream ofs(_fn_out);
			if (! ofs.is_open())
				THROW(boost::format("Unable to open file %s") % _fn_out);
			Stat::GenCDF<double>(_initial_max_temperature, ofs);
			ofs.close();
