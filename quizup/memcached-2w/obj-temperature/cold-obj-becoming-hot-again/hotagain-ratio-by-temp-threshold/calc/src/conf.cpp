	...
	void _ParseArgs(int argc, char* argv[]) {
		...
		od.add_options()
			("become_hotagain_temp_threshold",
			 po::value<double>()->default_value(Conf::Get("become_hotagain_temp_threshold").as<double>()))
		...
		__EditYaml<double>("become_hotagain_temp_threshold", vm);
	}

	void Init(int argc, char* argv[]) {
		Cons::MT _("Parsing conf ...", false);
		_LoadYaml();
