...
class CompInfo:
  ...
  ttssm_in_sst_ids = set()

  @staticmethod
  def Init():
    CompInfo.jobid_compinfo = {}
    CompInfo.ttssm_in_sst_ids = set()

  @staticmethod
  def AddOutSstInfo(j1):
    job_id = int(j1["job"])
  ...
  @staticmethod
  def SetTempTriggeredSingleSstMigr(line):
    mo = re.match(r"(?P<ts>(\d|\/|-|:|\.)+) .+EVENT_LOG_v1 (?P<json>.+)", line)
    if mo is None:
      raise RuntimeError("Unexpected: [%s]" % line)
    j = mo.group("json")
    ...
    if in_path_id == out_path_id:
      return
    CompInfo.ttssm_in_sst_ids.add(in_sst_id)
  ...
  @staticmethod
  def TempTriggeredSingleSstMigr(job_id):
    if job_id not in CompInfo.jobid_compinfo:
      return False
    ...
    for in_sst_id in ci.in_sst_ids:
      ttssm1 = (in_sst_id in CompInfo.ttssm_in_sst_ids)
  ...
  @staticmethod
  def MigrDirc(job_id, out_sst_id):
    ci = CompInfo.jobid_compinfo[job_id]
    osi = ci.out_ssts[out_sst_id]
