...
if (1) {
  ...
  set y2label "# of SSTables" tc rgb "blue"
  ...
  plot \
  ROCKSDB u 1:4:3:(0)       w vectors axes x1y2 nohead lc rgb C_NUM_SSTS lw LW not, \
  ROCKSDB u 2:4:(0):($5-$4) w vectors axes x1y2 nohead lc rgb C_NUM_SSTS lw LW not, \
  CPU_STAT u (x0($1)):(column(b0+3)):(column(b0+2)):(column(b0+6)):(column(b0+5)) w candlesticks whiskerbars lc rgb C_U lw LW not, \
  CPU_STAT u (x0($1)):(column(b0)) w p pt 7 ps PS lc rgb C_U not, \
  CPU_STAT u (x1($1)):(column(b1+3)):(column(b1+2)):(column(b1+6)):(column(b1+5)) w candlesticks whiskerbars lc rgb C_C lw LW not, \
