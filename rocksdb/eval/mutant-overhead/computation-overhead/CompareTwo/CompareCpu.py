...
def GetOverhead(exp_rocksdb, exp_computation):
  ...
  # cpu-1minavg-180201-032816.669-180201-032843.423
  fn = "%s/cpu-1minavg-%s" % (Conf.GetOutDir(), "-".join(exp_dts))
  with open(fn) as fo:
    for line in fo:
...
def Get1minAvgFn(exp_rocksdb, exp_computation):
  ...
  with Cons.MT("Creating avg cpu usage comparison file for plotting ..."):
    ...
    # {which_exp, [cpu_usage]}
    which_cpusum = {0: [], 1: []}
    hm_cpu = _GetHmCpu(exp_rocksdb)
    for hm, cpu in hm_cpu.iteritems():
      records.append(_RecordCpuAvg(hm, 0, cpu, 0))
      which_cpusum[0].append(cpu)
    hm_cpu = _GetHmCpu(exp_computation)
    for hm, cpu in hm_cpu.iteritems():
      records.append(_RecordCpuAvg(hm, 30, cpu, 1))
      which_cpusum[1].append(cpu)
    records.sort(key=operator.attrgetter("ts"))

  fmt = "%8s %6.2f %1d"
  header = Util.BuildHeader(fmt, "timestamp cpu_avg exp_type")
  with open(fn_out, "w") as fo:
    fo.write("# CPU usage * time (percent * sec)\n")
    r = sum(which_cpusum[0])
    c = sum(which_cpusum[1])
    fo.write("#   RocksDB: %f\n" % r)
