...
_exp_list_mutant_by_mig_temp_thrd = None
_exp_rocksdb_simulation_time_begin = None


def GetStgSizeCostData():
	fn_out = "%s/../.result/2level-mutant-stg-sizetime-cost-by-sstmigtempthresholds" % os.path.dirname(__file__)
	...
	global _exp_list_mutant_by_mig_temp_thrd
	...
	_exp_list_mutant_by_mig_temp_thrd = exp_list_root["Mutant"]["By SSTable migration temperature thresholds"]
	if False:
		for sst_mig_temp_threshold, exps in _exp_list_mutant_by_mig_temp_thrd.iteritems():
...
def GetUnmodifiedRocksDBSizeCostData():
	fn = "%s/../.result/2level-mutant-stg-sizetime-cost-by-sstmigtempthresholds" % os.path.dirname(__file__)
	with open(fn) as fo:
		...
		for line in fo:
			if line.startswith("#"):
				...
				if len(t) == 12 and t[1] == "RocksDB":
					...
					# Calculate EBS MAG cost using the EBS SSD experiments
					cost_ebs_mag = cost_local_ssd * PRICE_EBS_MAG / PRICE_LOCAL_SSD

					return (cost_local_ssd, cost_ebs_ssd, cost_ebs_mag,  size_time)
...
def _CalcCostAll():
	with Cons.MT("Calculating all cost ..."):
		if True:
			...
			for k, exps in _exp_list_mutant_by_mig_temp_thrd.iteritems():
				for e in exps:
					Util.RunSubp("%s/../calc/calc.py --simulation_time_begin=%s" % (os.path.dirname(__file__), e))
			Util.RunSubp("%s/../calc/calc.py --simulation_time_begin=%s" % (os.path.dirname(__file__), _exp_rocksdb_simulation_time_begin))
...
def _GetSizetimeCostByStgdev(simulation_time_begin):
	st_cost_by_stgdev = SizetimeCostByStgdev(simulation_time_begin)

	fn = "%s/../.result/%s/data-size-by-stg-devs-by-time" % (os.path.dirname(__file__), simulation_time_begin)
	with open(fn) as fo:
		while True:
			line = fo.readline()
			if not line:
			...
			if line == "# Data size-time (GB*Month):\n":
				for i in range(5):
					...
					st_cost_by_stgdev.SetSizetime(dev, v)
				...
				for i in range(5):
					...
					st_cost_by_stgdev.SetCost(dev, v)
	return st_cost_by_stgdev
...
class SizetimeCostByStgdev:
	def __init__(self, simulation_time_begin):
		...
		self.dev_sizetime = {}
		self.dev_cost = {}
	def SetSizetime(self, dev, v):
		self.dev_sizetime[dev] = v
	def SetCost(self, dev, v):
		self.dev_cost[dev] = v
