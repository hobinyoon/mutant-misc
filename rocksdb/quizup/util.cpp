...
namespace Util {
...
#if 0
	string ToString(const boost::posix_time::time_duration& td) {
		// in: 7027:33:44
		// out: 1y0m3d-19:33:44
	...
	string ToString(const boost::posix_time::ptime& pt) {
		{
			if (pt.is_not_a_date_time()) {
				...
				return s.substr(2, 6) + "-" + s.substr(9, 10);
			}
		}

		// C++ way. Doesn't seem very efficient.
		if (false) {
			stringstream ss;
			boost::posix_time::time_facet* outFacet = new boost::posix_time::time_facet("%y%m%d-%H%M%s");
	...
	// Input in the format of 160711-170502.871000
	// Doesn't seem very efficient. Make sure it's not called frequently.
	boost::posix_time::ptime ToPtime(const string& s) {
		stringstream ss;
		// You don't need to delete time_input_facet.
		// http://stackoverflow.com/questions/17779660/who-is-responsible-for-deleting-the-facet
		//
		// Note on the small s in %s for the microsecond resolution
		ss.imbue(locale(ss.getloc(), new boost::posix_time::time_input_facet("%y%m%d-%H%M%s")));
		ss.str(s);
	...
	// Input in the format of 160711170502871000
	boost::posix_time::ptime ToPtime(const long l) {
		// 160711170502871000
		//      1000000000000

		//           160711170502871000
		int y  = (l / 10000000000000000L);
		int m  = (l % 10000000000000000L) / 100000000000000L;
		int d  = (l %   100000000000000L) /   1000000000000L;
		int H  = (l %     1000000000000L) /     10000000000L;
		int M  = (l %       10000000000L) /       100000000L;
		int S  = (l %         100000000L) /         1000000L;
		int ms = (l %           1000000L);

		// Month and day in date() starts from 1.
		return boost::posix_time::ptime(boost::gregorian::date(2000 + y, m, d),
	...
	string CurDateTimeStr() {
		...
		char       buf[20];
		strftime(buf, sizeof(buf), "%y%m%d-%H%M", &tm_);
	...
	string CurDateTimeStr() {
		...
		time_t now_in_time_t = chrono::system_clock::to_time_t(now);
	...
	vector<string> ListDir(const string& dn) {
		...
		for (fs::directory_iterator dir_itr(p); dir_itr != end_iter; ++dir_itr) {
			try {
				if (fs::is_regular_file(dir_itr->status())) {
					//fns.push_back(dir_itr->path().filename());
					fns.push_back(dir_itr->path().string());
				}
	...
	const string& Hostname() {
		static string hn;
		if (hn.size() == 0) {
			char hn0[HOST_NAME_MAX];
			gethostname(hn0, HOST_NAME_MAX);
			hn = hn0;
		}
		return hn;
	}
